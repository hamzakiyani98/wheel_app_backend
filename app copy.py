...
def refresh_token():
    ...
    cursor.execute("UPDATE spin_user_tokens SET revoked = 1 WHERE token = %s", (refresh_token,))
    conn.commit()
    cursor.close()
    conn.close()
...
def login_email():
    ...
    cursor.close()
    conn.close()
    return jsonify({"success": True, "email": user_info[0], "name": user_info[1]})
...
def get_user():
    if "email" in session:
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute("SELECT last_wheel_id FROM Spin_users WHERE email = %s", (session["email"],))
        last_wheel_id_result = cursor.fetchone()
        last_wheel_id = last_wheel_id_result[0] if last_wheel_id_result else None
        current_wheel = None
        if last_wheel_id:
...
def get_wheels():
    ...
    for w in wheels:
        ...
        wheel_data.append({
            ...
            "settings": settings,
            "created_at": str(w[4]),
            "share_uuid": w[5],  # Add share_uuid here
            "images": [{"url": img[0], "type": img[1]} for img in images]
        })
    cursor.close()
    conn.close()
...
def update_current_wheel():
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT id, last_wheel_id FROM Spin_users WHERE email = %s", (session["email"],))
        user = cursor.fetchone()
        if not user:
            cursor.close()
            conn.close()
...
def submit_feedback():
    ...
    cursor.execute("""
        ...
        VALUES (%s, %s, %s)
    """, (user_id, session["email"], feedback))
